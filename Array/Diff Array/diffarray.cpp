...
public:
    Difference(vector<int> &nums)
    {
        int n = nums.size();
        diff.resize(n);
        diff[0] = nums[0]; // 差分数组默认第一项相同
        for (int i = 1; i < n; i++)
        {
            diff[i] = nums[i] - nums[i - 1]; // 数组除第一项外，每个元素都是原数组元素 后-前
        }
    }
    ...
    {
        // 原数组某一部分元素同加同减
        // 对于diff数组，diff[i]变化，则i...之后的元素都进行相同的变化
        // 但是在j + 1...之后的数据不应该变化，则diff[j + 1]要进行反操作
        diff[i] += val;
        if (j + 1 < diff.size())
            diff[j + 1] -= val;
    }
    ...
    vector<int> result()
    {
        ...
        {
            res[i] = diff[i] + res[i - 1]; // 由构建diff公式得
        }
        return res;
    }
};
